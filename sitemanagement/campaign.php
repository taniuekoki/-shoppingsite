	...
	while(true){
		if(isset($_POST["hold_btn"])){
			...
			if($_POST["hold_btn"] == "キャンペーン開催"){
				// //開催時(insert)
				//discountテーブルの処理
				$sql = "insert into k2g2_discount values(? , ? , ? , ? ,'campaign_bunner.".$file_extension."')";
				...
				//更新時(update)
				//discountテーブルの処理
				$sql = "update k2g2_discount SET campaign_name = ?,sale_ratio= ?,sale_limit= ?,sale_detail = ? ".$file_sql;
				$result = db_execution($sql,$arr);
				if(!$result){
					$err_flg = 1;
					break;//rollbackに向かう
				...
				$sql2 = "update k2g2_item set sale_flag = 1 ,sale_limit = ? where item_code IN (select item_code from(".$item_sql[0].")as b)";
				$result = db_execution($sql2,$arr2);
				if(!$result){
					$err_flg = 1;
					break;//rollbackに向かう
			...
			//キャンペーン削除ボタン処理
			$arr = array();
			$sql = "update k2g2_item set sale_flag = 0 ,sale_limit = 0";
			$result = db_execution($sql,$arr);
			if(!$result){
				$err_flg = 1;
				break;//rollbackに向かう
			...
			//discountテーブル
			$arr2 = array();
			$sql2 = "delete from k2g2_discount";
			$result = db_execution($sql2,$arr2);
...
?>
			<div class="cam_editer">
				<form name="k2" action='<?= $_SERVER["SCRIPT_NAME"]?>' method="post" enctype="multipart/form-data" id="cam_form">
					<label>キャンペーン名</label>
					<input type="text" name="cam_name" id="cam_name" value="<?=$campaing_name?>">
					<p class="is-error-cam_name err_msg"></p>
					<div class="line"></div>
					<label>内容</label>
					<input type="text" name="cam_detail" id="cam_detail" value="<?= $sale_detail?>">
					<p class="is-error-cam_detail err_msg"></p>
					<div class="line"></div>
					<div class="center">
						<input type="hidden" name="MAX_FILE_SIZE" value="3145728">
						<label>バナー</label>
            <input type="checkbox" name="checked" value="1" <?php if($sale_bunner){ echo 'checked="checked"';}else{ echo 'disabled="disabled"';}?> id="image_check1"/>
						<small>現在の画像を使用</small><br />
<?php
if($sale_bunner){
	$sale_bunner_url = "../images/campaigns/".$sale_bunner;
}else{
	$sale_bunner_url = "../images/campaigns/blank.png";
}
?>
            <img src="<?= $sale_bunner_url ?>" class="uploaded thumb" alt="" /><br>
            <input type="file" name="images" />
						<p class="is-error-images err_msg"></p>
					</div>
					<div class="line"></div>
					<label>割引率(1～100)</label>
					<input type="text" name="cam_ratio" id="cam_ratio" value="<?= $sale_ratio?>"><span id="parsent">&#37;</span>
					<p class="is-error-cam_ratio err_msg"></p>
					<div class="line"></div>
					<label>割引限定数残り(1～10000)</label>
					<input type="text" name="cam_left_count" id="cam_left_count" value="<?= $sale_limit?>">
					<p class="is-error-cam_left_count err_msg"></p>
					<div class="line"></div>
					...
					<div class="item_select_wrap">
						<div class="flex center">
							...
							<input type="text" class="input_price" name="item_price_low" size="10" value="1">円～
					...
					<div class="campaign_target_product_wrap"><br>
						...
						<div class="line"></div>
						<input type="submit" name="delete_btn" value="キャンペーン削除" form="delete_form">
						<input type="submit" name="hold_btn" value="<?= $sale_display ?>">
						<div class="line"></div>
				...
				<form  action='<?= $_SERVER["SCRIPT_NAME"]?>' method="post" id="delete_form">
